...
Tests:
1. Differentiate polynomials exactly up to degree N.
2. Differentiate exp(-y) on the mapped domain [0, 40].
3. Verify domain mapping properties.
4. Check second-derivative accuracy for smooth physical profiles.
"""
...
def test_exp_on_mapped_domain():
    ...
    y, D1, _ = physical_derivatives(D_eta, y_max=40.0, N=60)
    f = np.exp(-y)
...
def test_domain_mapping():
    ...
    print('  Metric dy/deta > 0: OK')
    print('  PASSED\n')


def test_second_derivative_accuracy():
    """Check second derivatives on the physical domain for smooth profiles."""
