...
def plot_sean():
    ...
    # sort by frequency for clean lines
    si = np.argsort(f_pm)
    ...
    # reference -- dashed + hollow markers
    ...
    ax.annotate(
        ...
        xy=(0.03, 0.03), xycoords="axes fraction", fontsize=11.5,
        va="bottom", ha="left",
        bbox=dict(boxstyle="round,pad=0.4", fc="#f2f2f2", ec=GREY, alpha=0.95))
...
def plot_mazhong():
    ...
    # shade unstable band (sigma>0)
    ax.axhspan(0, max(sigma) * 1.15, xmin=0, xmax=0, alpha=0)  # noop to set scale
    ...
    # pyMack neutral crossings (solid vertical lines)
    ...
    # Ma & Zhong reference (dashed vertical lines)
    ...
    ax.annotate(
        ...
        "topology: one closed band, two neutral pts",
        xy=(0.03, 0.04), xycoords="axes fraction", fontsize=11.5,
        va="bottom", ha="left",
        bbox=dict(boxstyle="round,pad=0.4", fc="#f2f2f2", ec=GREY, alpha=0.95))
...
def plot_mack_fig10_1(mtag, mlabel, headline):
    ...
    # reference complete-equations: split by branch
    R_ce = fcol(ce, "R")
    F_ce = fcol(ce, "F_x1e4")
    ...
    # deprecated old (Dunn-Lin) ref
    x_dep = fcol(dep, "x")
    y_dep = fcol(dep, "y")

    fig, ax = plt.subplots(figsize=(8.6, 6.8))

    # deprecated old ref -- faint dotted grey
    sd = np.argsort(x_dep)
    ax.plot(x_dep[sd], y_dep[sd], ":", color=GREY, lw=1.6, alpha=0.8,
            label="old (Dunn-Lin) ref [deprecated]")

    # Mack complete-equations loop -- dashed orange, hollow markers
    ...
    # pyMack -- solid lines, filled markers
...
def plot_ozgen(m, headline):
    ...
    # build c_i grid mesh for faint unstable contour
    gRe = fcol(grid, "Re_L")
    ...
    # faint c_i>0 unstable lobe (pyMack)
    Zm = np.ma.masked_invalid(Z)
    try:
        cf = ax.contourf(RR, AA, Zm, levels=[0, Zm.max()],
    ...
    # pyMack neutral locus -- solid, filled
    ...
    ax.set_xlim(left=0)
    ax.set_ylim(bottom=0)
    ax.legend(handles, labels, loc="upper right", framealpha=0.92, fontsize=11)

    ax.annotate(
        "open-lobe (pyMack) vs closed-arch (paper) topology",
        xy=(0.03, 0.04), xycoords="axes fraction", fontsize=11,
        va="bottom", ha="left",
        bbox=dict(boxstyle="round,pad=0.35", fc="#f2f2f2", ec=GREY, alpha=0.95))
