"""Overlay Our LST code's full first- and second-mode neutral curves (neg_alpha_i=0
...
def first_cutoff_discrete():
    """Our LST code's first-mode CUTOFF branch from the discrete-mode extractor
...
def main():
    ...
    ax.text(70, 0.030, "1st-mode onset:\ncontinuous-spectrum-limited",
            fontsize=13, color="0.35", zorder=1)

    # Our LST code (BLACK): 2nd-mode neutral loop dashed, 1st-mode cutoff dash-dot
    # (same colour; the two modes distinguished by line style).
    firstlbl = True
    for s in contour0(*g["second"]):
        ax.plot(s[:, 0], s[:, 1], "--", color="#000000", lw=3.6,
                label=("Our LST code 2nd-mode neutral" if firstlbl else None)); firstlbl = False
    fc = first_cutoff_discrete()
    if fc is not None:
        ax.plot(fc[:, 0], fc[:, 1], "-.", color="#000000", lw=3.6,
                label="Our LST code 1st-mode cutoff")

    # Ma & Zhong digitised reference points -> RED, larger; mode by marker
    for (m, br), arr in oz.items():
        a = np.array(sorted(arr.tolist()))
        mk = "s" if m == "second" else "o"
        ax.plot(a[:, 0], a[:, 1], mk, mfc="none", mec="#d62728", mew=1.4, ms=5, zorder=3)
    ax.plot([], [], "s", mfc="none", mec="#d62728", mew=1.4, ms=5, label="Ma & Zhong 2nd mode (digitized)")
    ax.plot([], [], "o", mfc="none", mec="#d62728", mew=1.4, ms=5, label="Ma & Zhong 1st mode (digitized)")

    Rl = np.linspace(0, 2000, 50)
    ax.plot(Rl, Rl * 2.2e-4, ":", color="0.45", lw=1.4)
    ax.text(1250, 0.258, r"$F=2.2\times10^{-4}$", fontsize=13)
    ax.set_xlim(0, 2000); ax.set_ylim(0, 0.28)
    ax.set_xlabel(r"$R=\sqrt{Re_x}$", fontsize=19); ax.set_ylabel(r"$\omega$", fontsize=19)
    ax.set_title("Ma & Zhong (2003) Fig. 15:  $M=4.5$ neutral curves", fontsize=17)
    ax.tick_params(labelsize=16); ax.legend(fontsize=13.5, loc="lower right"); ax.grid(True, alpha=0.25)
    fig.tight_layout(); fig.savefig(HERE / "overlay_fig15_full.png", dpi=160)
    print("wrote overlay_fig15_full.png")
    # report the F=2.2e-4 crossings of Our LST code 2nd-mode neutral (should ~ branch I/II)
    R, om, Z = g["second"]
    print("Our LST code 2nd-mode unstable omega range per R (neutral band edges):")
