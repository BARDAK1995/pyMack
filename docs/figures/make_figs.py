...
def fig_baseflow():
    ...
    axT.annotate("hot near-wall layer\n(viscous heating)", xy=(T[0], 0.05),
                 xytext=(T.max() * 0.38, 1.4), color=HOT, fontsize=10.5,
                 arrowprops=dict(arrowstyle="-|>", color=HOT, lw=1.2))
    if yc is not None:
        ax.axhline(yc, color="#444", ls=":", lw=1.5)
        ax.text(0.06, yc + 0.12, r"gen. inflection point: $(\overline{\rho}\,\overline{U}\,\!\!^\prime)^\prime=0$",
                fontsize=10, color="#444")
        ax.text(0.06, yc - 0.45, "(the inviscid-instability site)", fontsize=9.5, color="#444")
    ax.text(0.55, 7.2, r"$M_e=4.5$, adiabatic", fontsize=12)
    ax.grid(alpha=0.25)
...
def fig_nfactor():
    ...
    sigma = 0.06 * np.exp(-((x - 6.0) ** 2) / 5.5) - 0.004
    ...
    ax2.text(0.3, N.max() + 0.003, f"$e^N$ amplification (here $N\\approx{N.max():.2f}$)",
             fontsize=11.5, color=REF)
