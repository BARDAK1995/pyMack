...
\paragraph{From the governing equations to the matrix problem the code solves.} Part~III, and the two
Parts after it, are a single chain of transformations carrying the \emph{full governing equations} (top ---
nonlinear, unsolvable by hand) into the \emph{one matrix problem the code actually solves} (bottom ---
boxed). Those two are the forms worth remembering; everything in between is how we get from the first to the
last.
...
{\footnotesize four coupled \textbf{ODEs in $y$} for the wall-normal shape $\hatq=(\hatu,\hatv,\hatT,\hatp)$ --- Eqs.~\eqref{eq:cont}--\eqref{eq:energy}}\\[6pt]
{\footnotesize$\downarrow$\quad Chebyshev collocation: $\D\!\to\!D_1,\ \D^2\!\to\!D_2$, each mean coefficient $\to$ a diagonal matrix, the four fields stacked into one vector\quad(\S\ref{sec:cheb}--\ref{sec:assemble})}\\[6pt]
$\boxed{\,A(\alpha)\,\boldsymbol\phi=c\,B(\alpha)\,\boldsymbol\phi\,}\qquad A,B\in\mathbb C^{4n\times4n}$\\[1pt]
{\footnotesize the \textbf{discrete generalised eigenvalue problem} \texttt{temporal\_solver.py} assembles and solves --- Eq.~\eqref{eq:tEVP}}
\end{center}
...
$\alpha$ real and solving for $c$ is the \emph{temporal} problem (\S\ref{sec:temporal}); holding $\omega$
real and regrouping the same ODEs by powers of $\alpha$ turns the last line into the \emph{spatial}
quadratic problem (\S\ref{sec:spatial}). The rest of Part~III derives the two middle lines; Parts~IV--V
derive the last.
...
\paragraph{The construction recipe (how a term becomes a matrix entry).} Turning an equation into blocks is
mechanical: \emph{(i)} every mean coefficient $f(y)$ becomes the diagonal $\operatorname{diag}\!\big(f(y_j)\big)$;
\emph{(ii)} $\D\!\to\!D_1$ and $\D^2\!\to\!D_2$, so a term $f\,\D$ becomes $\operatorname{diag}(f)\,D_1$
(coefficient times derivative matrix, in that order); \emph{(iii)} the constants $\imag\alpha$ (from
$\partial_x$) and $-\alpha^2$ (from $\partial_x^2$) multiply through; \emph{(iv)} the result is deposited in the
block at \emph{(its equation's row, its field's column)}; \emph{(v)} anything carrying the eigenvalue $c$ is
stripped of $c$ and placed in $B$ at the same address --- everything else stays in $A$. Four equations
$\times$ four fields $=$ a $4\times4$ grid of $n\times n$ blocks per matrix.
...
\end{equation}
\paragraph{Worked example --- the continuity block-row, built term by term.} Apply the recipe to
Eq.~\eqref{eq:cont}, splitting the convective factor $\imag\alpha(\bar U-c)=\imag\alpha\bar U-c\,\imag\alpha$
...
with $\mathcal C_\kappa,\mathcal C_d,\mathcal L_c$ as defined under Eq.~\eqref{eq:energy}, and a bare mean
symbol denotes its diagonal matrix. The five $B$-blocks are Eq.~\eqref{eq:Bblocks}; \emph{every block not
listed is zero} --- notably $A_{(E,\hatp)}=0$, since the energy row has no pressure term.
{\footnotesize
